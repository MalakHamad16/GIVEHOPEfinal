...
function renderCases(filter) {
  ...
  let filtered = filter === "all" ? casesData : casesData.filter(c => c.type === filter);
  ...
  filtered.forEach(caseItem => {
    ...
    let sponsorBtn;
    if (isFully) {
      sponsorBtn = `<button class="btn btn-success disabled" style="opacity:0.7; pointer-events:none;">
        <i class="fas fa-check"></i> تم الكفالة
      </button>`;
    } else if (isPartial) {
      if (currentUser && caseItem.sponsorId && currentUser.id === caseItem.sponsorId) {
        sponsorBtn = `<a href="DonateNow.html?type=sponsor&id=${caseItem._id}"
          class="btn btn-primary">
          <i class="fas fa-hand-holding-usd"></i> ادفع الدفعة
        </a>`;
      } else {
        sponsorBtn = `<button class="btn btn-primary disabled" style="opacity:0.6; pointer-events:none;">
